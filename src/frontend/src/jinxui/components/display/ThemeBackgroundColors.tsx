...
const gradient = ({ theme, index }: { theme: Theme; index: number }) => {
  ...
  const type = "main";
  const [first, last] =
    index % 2 === 0 ? ["primary", "secondary"] : ["secondary", "primary"];
...
export const rainbowGradient = ({ index }: { index: number }) => {
  const [firstColor, lastColor] = [
    hues[index % hues.length]["A100"],
    hues[(index + 1) % hues.length]["A100"],
  ];
  const backgroundColor = `linear-gradient(${firstColor} 0%, ${lastColor} 100%)`;
  const textColor = "#000";
  const isFullHeight = false;
...
const alternatingBackgroundColors = ({
  theme,
  index,
}: {
  theme: Theme;
